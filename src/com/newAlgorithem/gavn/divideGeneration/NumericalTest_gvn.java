...
public class NumericalTest_gvn {
	public static void main(String[] args) {
        ...
        if (args.length==1){
    		...
    		if (args[0].trim().toLowerCase().equals("dg")){
    			String head=buildFileName();
    			for(int j = 0; j <  DivideGeneration.RunTime; j++){
    				String dic = "data/DivideGeneration"+head+"/divide_"+j;
	   				File f=new File(dic);
	               	if(f.exists()) {
	               		 f.delete();
	               	}
    				...
    				for(int i = 0; i<fl.length; i++){
                   	String _fn =  "case_def/" + fl[i];
                   	String _fo = dic+"//divide_"+fl[i]+".txt";
                   	DivideGeneration_algorithm(_fn,_fo,countResult);
                   }
    			}
                System.out.println(fl.length +"个案例计算完成"); 
                return;
	 ...
	 * 分成三个种群 
	 */
	private static void DivideGeneration_algorithm(String casefile,String datafile, List<List<Double>> countResult) {
		//记录开始计算的时间，用于统计本算法的总时间
		long startTime = System.currentTimeMillis();
		// 创建案例类对象
		Case project = new Case(casefile);
		// 初始化种群
		Population P = new Population(DivideGeneration.populationSize,project,true);
		long time=0;
		long t2 = 0;
		P.dividePopulation();
		while (time < 30 ) {
			P = P.getOffSpring_DivideGeneration();
			t2=System.currentTimeMillis();
			time=(t2-startTime)/1000;
		}
		//从最后得到种群中获取最优解集
		P=P.merged(P.getOffSpring_duration(),P.getOffSpring_cost(),P.getOffSpring_banlance());
